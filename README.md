# integration-test-react-node-mongo

## instruction
yarn
yarn start

register three users accordingly.
...
Then
yarn run test:watch
yarn run test is currently not working due to well known babel-eslint bug.
one integration test on frontend,
one server test on backend
two tests succesfully passes.

# requirements
dockerize,
heroku,
show students' grades
professors see students grades
